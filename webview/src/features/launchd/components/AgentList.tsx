...
export function StatusDot({ state, title }: { state: LaunchAgentState; title?: string }) {
	return <span className={`size-2 shrink-0 rounded-full ring-1 ring-[color-mix(in_srgb,currentColor_18%,transparent)] ${stateClassNames[state]}`} title={title} />;
}
...
export function AgentList({ agents, selectedFileName, busy, detailsLoading, onSelect, onAction, onDetails, onRemove }: {
	...
	return (
		<aside className="border-r border-(--powerkit-border) bg-(--vscode-sideBar-background) max-[760px]:max-h-77.5 max-[760px]:overflow-auto max-[760px]:border-r-0 max-[760px]:border-b" aria-label="LaunchAgents">
			<div className="sticky top-0 z-10 flex min-h-11 items-center justify-between border-b border-(--powerkit-border) bg-(--vscode-sideBar-background) px-3.5 text-[11px] font-semibold uppercase text-(--vscode-sideBarSectionHeader-foreground,var(--vscode-foreground))"><span>Agents</span><span className="grid h-5 min-w-5 place-items-center rounded-full bg-(--vscode-badge-background) px-1.5 text-[10px] font-normal text-(--vscode-badge-foreground)">{agents.length}</span></div>
			{busy && agents.length === 0 ? <EmptyState icon={<LoaderCircle className="animate-spin" size={22} />} label="Scanning LaunchAgents" /> : agents.length === 0 ? <EmptyState icon={<Rocket size={22} />} label="No LaunchAgents found" /> : agents.map(agent => {
				...
				return (
					<article key={agent.fileName} className={`group relative flex min-h-14.5 items-center justify-between gap-2 border-b border-(--powerkit-border-subtle) py-2 pr-2 pl-3.5 transition-colors duration-75 hover:bg-(--vscode-list-hoverBackground) ${selected ? 'bg-(--vscode-list-activeSelectionBackground) text-(--vscode-list-activeSelectionForeground) before:absolute before:inset-y-0 before:left-0 before:w-0.5 before:bg-(--vscode-focusBorder)' : ''}`} onClick={() => onSelect(agent)}>
...
function RowButton({ className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
	return <button className={`grid size-6.5 place-items-center rounded-xs border-0 bg-transparent text-inherit hover:bg-(--vscode-toolbar-hoverBackground) disabled:cursor-default disabled:opacity-45 ${className}`} {...props} />;
}
