...
export function LaunchdDetails({ loading, details, onClose }: { loading: boolean; details?: LaunchAgentDetails; onClose(): void }) {
	return (
		<div className="fixed inset-0 z-20 flex justify-end bg-black/30 backdrop-blur-[1px]" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
			<section className="h-full w-[min(620px,calc(100%-32px))] overflow-auto border-l border-(--vscode-panel-border) bg-(--vscode-editor-background) shadow-[-16px_0_32px_var(--vscode-widget-shadow)] max-[760px]:w-full" role="dialog" aria-modal="true" aria-label="LaunchAgent runtime details">
...
function DetailsContent({ details }: { details: LaunchAgentDetails }) {
	return <div className="p-5"><div className="mb-4 flex items-center gap-2 rounded-xs border border-(--vscode-panel-border) bg-(--vscode-editorWidget-background) px-3 py-2.5 text-xs"><StatusDot state={details.state} /><strong>{stateText(details.state)}</strong></div><dl className="m-0 grid grid-cols-2 gap-px overflow-hidden rounded-xs border border-(--vscode-panel-border) bg-(--vscode-tree-tableColumnsBorder) max-[760px]:grid-cols-1"><Detail label="PID" value={details.pid} /><Detail label="Active count" value={details.activeCount} /><Detail label="Runs" value={details.runs} /><Detail label="Last exit code" value={details.lastExitCode} /><Detail label="Program" value={details.program} wide /><Detail label="Working directory" value={details.workingDirectory} wide /><Detail label="Standard output" value={details.standardOutPath} wide /><Detail label="Standard error" value={details.standardErrorPath} wide /><Detail label="Termination reason" value={details.reason} wide /></dl><div className="mt-4 overflow-hidden rounded-xs border border-(--vscode-panel-border)"><h3 className="m-0 border-b border-(--vscode-panel-border) bg-(--vscode-editorWidget-background) px-3 py-2.5 text-[11px] font-semibold">launchctl print</h3><pre className="m-0 max-h-110 overflow-auto bg-(--vscode-textCodeBlock-background) p-3 font-(--vscode-editor-font-family) text-[11px] leading-5 whitespace-pre text-(--vscode-editor-foreground)">{details.raw}</pre></div></div>;
}
